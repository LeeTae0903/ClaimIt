-- Giveaways: one deposit funding many links.
...
-- A batch deposit belongs to the batch rather than to any one link, so a
-- transaction now hangs off exactly one of the two.
...
-- Enforce the "exactly one owner" rule in the database rather than trusting
-- every future call site to remember it.
